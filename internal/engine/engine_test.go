...
func TestDerivePhase_ZeroIsBan1(t *testing.T) {
	phase := DerivePhase(0)

	if phase != PhaseBan1 {
		t.Fatalf("Expected PhaseBan1, got %v", phase)
	}
}

func TestHover_UpdatesHoverForSeat(t *testing.T) {
	s := NewEmptyState()
	s.Cursor = 6
	s.Picks = map[Team][]int{TeamBlue: {}, TeamRed: {}}
	cmd := Command{Type: CmdHoverChampion, Team: TeamBlue, SeatID: "Jack", ChampionID: 8}

	_, newState, _ := Apply(s, cmd)
	if newState.Hover["Jack"] != cmd.ChampionID {
		t.Fatalf("want %v, got %v", cmd.ChampionID, newState.Hover["Jack"])
	}
}

func TestHover_RejectsWhenNotPickTurn(t *testing.T) {
	s := NewEmptyState()
	s.Cursor = 7
	s.Picks = map[Team][]int{TeamBlue: {}, TeamRed: {}}
	cmd := Command{Type: CmdHoverChampion, Team: TeamBlue, SeatID: "Jack", ChampionID: 8}
...
func TestHover_IsTransient_NoEventsEmitted(t *testing.T) {
	s := NewEmptyState()
	s.Cursor = 6
	cmd := Command{Type: CmdHoverChampion, Team: TeamBlue, SeatID: "Jack", ChampionID: 8}

	events, _, _ := Apply(s, cmd)
	if events != nil {
		t.Fatalf("Expected empty events, got %v", events)
	}
}

func TestTimeoutAdvance_PicksHoveredChampion(t *testing.T) {
	s := NewEmptyState()
	s.Cursor = 6
	s.Hover = map[string]int{"Jack": 5}
	cmd := Command{Type: CmdTimeoutAdvance, SeatID: "Jack"}

	events, ns, err := Apply(s, cmd)
	if err != nil {
		t.Fatalf("unexpected: %v", err)
	}

	if !ContainsEvent(events, EvtChampionPicked) || !ContainsEvent(events, EvtTurnAdvanced) {
		t.Fatalf("expected EvtChampionBanned & EvtTurnAdvanced")
	}

	if ns.Picks[GameOrder[s.Cursor].Team] == nil {
		t.Fatalf("expected ns.Picks to contain hover: %v", ns.Picks[GameOrder[s.Cursor].Team])
	}
}

func TestTimeoutAdvance_PicksRandomWhenNoHover(t *testing.T) {
	old := chooseRandomLegal
	defer func() { chooseRandomLegal = old }()
	chooseRandomLegal = func(s State, team Team) (int, bool) { return 103, true }

	s := NewEmptyState()
	s.Cursor = 6
	cmd := Command{Type: CmdTimeoutAdvance}

	events, ns, err := Apply(s, cmd)
	if err != nil {
		t.Fatalf("unexpected: %v", err)
	}
	// assert events contain EvtChampionPicked (id 103) and TurnAdvanced, ns.Picks updated
	if !ContainsEvent(events, EvtChampionPicked) || !ContainsEvent(events, EvtTurnAdvanced) {
		t.Fatalf("expected EvtChampionBanned & EvtTurnAdvanced")
	}

	if ns.Picks[GameOrder[s.Cursor].Team] == nil {
		t.Fatalf("expected ns.Picks updated: %v", ns.Picks)
	}
}

func TestTimeoutAdvance_EmitsGameCompletedOnLastPick(t *testing.T) {
	old := chooseRandomLegal
	defer func() { chooseRandomLegal = old }()
	chooseRandomLegal = func(s State, team Team) (int, bool) { return 103, true }
	...
	if !ContainsEvent(events, EvtChampionPicked) || !ContainsEvent(events, EvtGameCompleted) {
		t.Fatalf("expected EvtChampionBanned & EvtGameCompleted: %v", events)
	}
}

func TestTimeoutAdvance_SkipsBanWhenNoHover(t *testing.T) {
	s := NewEmptyState()
	s.Cursor = 2
	cmd := Command{Type: CmdTimeoutAdvance}
	...
	if !ContainsEvent(events, EvtTurnAdvanced) && len(events) == 1 {
		t.Fatalf("expected EvtTurnAdvanced: %v", events)
	}
}
